...
const ExampleTwo = ({
  ...
  let updateHandler = async () => {
    if (
      ...
      let checkIsArray = Array.isArray(lobName);
      let LOb = [];
  ...
  const GetEmpDetails = async (EmployeeID) => {
    let result = await CommonService.httpGetService(
  ...
  const DeleteEMP = async (EmpCode) => {
    setShowModalDLT(false);
    let result = await CommonService.httpDeleteService(
      `deleteDataByID/${EmpCode}`
    );
    if (result.status == 200) {
      result = await result.json();
      toast.success(`${result.msg}`, {
      ...
      Cell: (row) => {
        ...
        return (
          <span className="block w-full">
            {loginID == row.cell.row.cells[1].value ? (
              <Tooltip
                content="Disable"
                placement="top"
                arrow
                ...
                theme="danger"
              >
                <div style={{ color: "red", width: "40%", height: "40%" }}>
                  <Icon icon="codicon:error-small" />
                </div>
              </Tooltip>
            ) : (
              <Switch
 ...
 let result = await CommonService.httpPostService(`getData`);
    result = await result.json();
                ...
                >
                  ...
                  >
                    ...
                    <Select
                      ...
                      styles={styles}
                      isMulti
                      value={lobNameList.filter((obj) =>
                        lobName.includes(obj.value)
                      )}
                      name="lob_Name"
                      options={lobNameList}
